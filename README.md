...
omen [--config PATH] \
     ...
     [--format {json,text,h1md,sarif,gha,junit,checkstyle,sonarqube,bitbucket-code-insights,azure-devops,teams-webhook,opsgenie,victorops}] \
...
- `--format` — `json` (machine-readable, default), `text` (a compact human-readable terminal summary — a per-severity count line plus one line per finding, worst-first; see [Text output](#text-output)), `h1md` (a HackerOne-style markdown report), `sarif` (a [SARIF 2.1.0](https://sarifweb.azurewebsites.net/) log for GitHub code scanning, VSCode, and CI ingestion), `gha` (GitHub Actions [workflow-command](https://docs.github.com/en/actions/using-workflows/workflow-commands-for-github-actions) annotations — `::error`/`::warning`/`::notice` lines the Actions runner turns into inline PR-diff annotations on **every** repo for free, with no Advanced-Security upload; see [GitHub Actions annotations](#github-actions-annotations)), `junit` (a [JUnit XML](https://github.com/testmoapp/junitxml) test-results report — one failing `<testcase>` per finding under a single `<testsuite>`, the platform-agnostic format ingested natively by GitHub Actions test reporters, GitLab CI, Jenkins, CircleCI, Azure DevOps, and TeamCity so omen findings land in the CI **tests** tab and fail the build on essentially any CI; see [JUnit XML output](#junit-xml-output)), `checkstyle` (a [Checkstyle XML](https://checkstyle.sourceforge.io/) document — one `<error>` per finding grouped under `<file>` elements, the static-analysis lingua franca ingested natively by GitLab CI's **Code Quality** widget, Reviewdog, SonarQube, and the Jenkins Checkstyle plugin so findings surface as **code-review annotations** in the MR/PR diff; see [Checkstyle XML output](#checkstyle-xml-output)), `sonarqube` (SonarQube's [Generic Issue Import Format](https://docs.sonarqube.org/latest/analyzing-source-code/importing-external-issues/generic-issue-import-format/) JSON — one issue per finding with SonarQube-native `BLOCKER`/`CRITICAL`/`MAJOR`/`MINOR`/`INFO` severity and a `VULNERABILITY` type, ingested directly via `sonar.externalIssuesReportPaths` so omen findings appear as first-class external issues in the SonarQube/SonarCloud UI; see [SonarQube generic issue output](#sonarqube-generic-issue-output)), or `bitbucket-code-insights` (Bitbucket Cloud's native [Code Insights](https://developer.atlassian.com/cloud/bitbucket/rest/api-group-reports/) JSON — one `report` block plus one annotation per finding with `CRITICAL`/`HIGH`/`MEDIUM`/`LOW` severity and a `VULNERABILITY` type, ingested directly via the Bitbucket Pipelines Code Insights REST API so omen findings render inline on the PR diff in the Bitbucket UI with no Atlassian-side custom importer; see [Bitbucket Code Insights output](#bitbucket-code-insights-output)), `azure-devops` (Azure DevOps Pipelines [logging-command](https://learn.microsoft.com/en-us/azure/devops/pipelines/scripts/logging-commands) `task.logissue` annotations — `##vso[task.logissue type=error;sourcepath=...;linenumber=...;code=...]message` lines the Azure Pipelines agent reads off stdout and turns into inline annotations on the build summary's **Files** tab and rows in the **Errors / Warnings** panel, on any Azure DevOps pipeline (cloud or Server) for free with no upload step, no Marketplace extension, and no paid tier; see [Azure DevOps pipeline annotations](#azure-devops-pipeline-annotations)), or `teams-webhook` (Microsoft Teams [Incoming Webhook](https://learn.microsoft.com/en-us/microsoftteams/platform/webhooks-and-connectors/how-to/add-incoming-webhook) [MessageCard](https://learn.microsoft.com/en-us/outlook/actionable-messages/message-card-reference) JSON — one card with a `themeColor` projected from the worst severity in the report plus one `section` per finding (severity / category / confidence / contract / detector / location facts + description body), piped straight to a Teams channel's Incoming Webhook URL with one `curl -d @-` with no Power Automate flow, no Azure Bot registration, and no Marketplace extension; see [Microsoft Teams webhook output](#microsoft-teams-webhook-output)), or `opsgenie` ([Opsgenie / Atlassian Ops Create Alert API](https://docs.opsgenie.com/docs/alert-api#create-alert) JSON — one alert with a `priority` P1..P5 projected from the worst severity, a markdown `description` carrying all findings, `omen:<category>` tags for routing rules, and a `details` map with scan metadata; POST to `https://api.opsgenie.com/v2/alerts` with an `Authorization: GenieKey` header; see [Opsgenie / Atlassian Ops alert output](#opsgenie--atlassian-ops-alert-output)), or `victorops` ([VictorOps / Splunk On-Call REST Endpoint](https://help.victorops.com/knowledge-base/victorops-restendpoint-integration/) JSON — one alert with `message_type` CRITICAL/WARNING/INFO/RECOVERY projected from the worst severity (RECOVERY for a clean scan, which auto-resolves the open alert), a stable `entity_id` for deduplication, and a plain-text `state_message` with all findings; POST to the routing-key URL; see [VictorOps / Splunk On-Call alert output](#victorops--splunk-on-call-alert-output)).
...
applies to single-`--contract` mode; a `--batch` run always emits its JSONL
stream.
### Bitbucket Code Insights output

`--format sarif` (above) targets GitHub Advanced Security; `--format checkstyle`
reaches GitLab CI's Code Quality widget; `--format sonarqube` lands in
SonarQube. The Bitbucket Cloud equivalent — the native review-side schema that
renders inline annotations on a Bitbucket PR diff — is the
[Code Insights](https://developer.atlassian.com/cloud/bitbucket/rest/api-group-reports/)
REST API. `--format bitbucket-code-insights` emits exactly that: a single JSON
document with a top-level `report` block (title, details, result, type,
reporter, per-severity counts) and an `annotations` array (one annotation per
finding, file/line-anchored, with the Bitbucket-native `CRITICAL`/`HIGH`/
`MEDIUM`/`LOW` severity and a `VULNERABILITY` annotation type). Bitbucket
Pipelines ingests the document directly — no Atlassian-side custom importer —
and Bitbucket lights up each annotation inline on the PR-diff in its UI.

```bash
omen --contract MyContract.sol --input-type sol --check all --format bitbucket-code-insights > omen-bitbucket.json
```
...
```json
{
  "report": {
    ...
    "result": "FAILED",
    "data": [
      {"title": "informational", "type": "NUMBER", "value": 0},
      {"title": "low", "type": "NUMBER", "value": 0},
      {"title": "medium", "type": "NUMBER", "value": 0},
  ...
  "annotations": [
    {
      "external_id": "omen-reentrancy|slither:reentrancy-eth|MyContract|MyContract.sol#42-47",
      "annotation_type": "VULNERABILITY",
      "summary": "[critical/reentrancy/high] external call before state update",
      "severity": "CRITICAL",
      "path": "MyContract.sol",
      "line": 42
    },
    {
      "external_id": "omen-tx-origin|slither:tx-origin|MyContract|MyContract.sol#12",
      "annotation_type": "VULNERABILITY",
      "summary": "[high/tx-origin/high] tx.origin used for authorization",
      "severity": "HIGH",
      "path": "MyContract.sol",
      "line": 12
    }
...
omen's five-level severity (`critical`/`high`/`medium`/`low`/`informational`)
projects onto Bitbucket's four-level annotation vocabulary
(`CRITICAL`/`HIGH`/`MEDIUM`/`LOW`) the natural way: `critical` → `CRITICAL`,
`high` → `HIGH`, `medium` → `MEDIUM`, `low` → `LOW`, and `informational` folds
into `LOW` (the same "you should know, but it's not a bug" bucket). The
original H1 severity is preserved verbatim in the annotation's `summary` line
(`[severity/category/confidence] description`), so the projection is not
actively lossy in the UI a reviewer reads — Bitbucket's four-level filter
exposes the projected level, but the verbatim H1 severity stays one click away.

The annotation's `external_id` is the finding's stable fingerprint
(`omen-<category>|<detector>|<contract>|<location>`), so re-running omen on
the same code **upserts** the same annotation via the Code Insights API rather
than duplicating it — the same fingerprint identity `--baseline`, `--diff`,
and `--sarif-baseline` use elsewhere in omen.

In a Bitbucket Pipelines workflow, run omen, then POST the report to the
Code Insights API:
      ...
      - step:
          ...
          script:
            - omen --contract MyContract.sol --input-type sol --check all \
                --format bitbucket-code-insights -o omen-bitbucket.json
            - |
              REPORT_ID="omen-$BITBUCKET_COMMIT"
              JQ_REPORT=$(jq '.report' omen-bitbucket.json)
              JQ_ANNS=$(jq '.annotations' omen-bitbucket.json)
              # PUT the report header
              curl -sf -X PUT \
                -H "Content-Type: application/json" \
                -u "$BITBUCKET_USER:$BITBUCKET_APP_PASSWORD" \
                "https://api.bitbucket.org/2.0/repositories/$BITBUCKET_REPO_FULL_NAME/commit/$BITBUCKET_COMMIT/reports/$REPORT_ID" \
                -d "$JQ_REPORT"
              # POST the annotations (bulk)
              curl -sf -X POST \
                -H "Content-Type: application/json" \
                -u "$BITBUCKET_USER:$BITBUCKET_APP_PASSWORD" \
                "https://api.bitbucket.org/2.0/repositories/$BITBUCKET_REPO_FULL_NAME/commit/$BITBUCKET_COMMIT/reports/$REPORT_ID/annotations" \
                -d "$JQ_ANNS"
```

A clean scan (no findings) emits a well-formed document with
`report.result: "PASSED"` and an empty `annotations` array, so the Bitbucket UI
shows a green report row rather than no report at all. A bytecode-mode finding
(no source mapping) and a source finding whose mapping lacks a line number
cannot be anchored to a PR-diff line; rather than dropping them, the
formatter emits them as report-level annotations (no `path`/`line`) so they
still appear in the Code Insights tab — the same fail-open posture the `gha`
...
`--format gha` / `--format azure-devops` / `--format gitlab-sast` /
`--format bitbucket-code-insights` all surface omen findings inside the
*build* UI of a CI platform. The complement is the *chat* surface: post the
scan result to a Microsoft Teams channel so the team sees a card the moment a
pipeline finds something, without anyone opening the build. Teams' [Incoming
Webhook](https://learn.microsoft.com/en-us/microsoftteams/platform/webhooks-and-connectors/how-to/add-incoming-webhook)
connector ingests a [legacy actionable MessageCard](https://learn.microsoft.com/en-us/outlook/actionable-messages/message-card-reference)
JSON payload directly — no Power Automate flow, no Azure Bot registration, no
Marketplace extension. `--format teams-webhook` emits exactly that payload, so
a CI step pipes the rendered JSON straight to the channel webhook URL with one
`curl`:

```bash
omen --contract MyContract.sol --input-type sol --check all --format teams-webhook \
  | curl -H 'Content-Type: application/json' -d @- "$TEAMS_WEBHOOK_URL"
```

The card carries:

- A top-level `summary` (the Teams notification preview text — "omen security
  scan: N findings"),
- A `themeColor` (the card's left-edge stripe) projected from the *worst*
  severity in the report: red for `critical` (`A6192E`) / `high` (`D13438`),
  amber (`F2C811`) for `medium`, purple (`5C2D91`) for `low`, gray (`8A8886`)
  for `informational`, and green (`107C10`) for an empty (clean) report — so a
  channel reader sees the worst-case signal at a glance before reading any
  text;
- A one-line scan summary in the `text` field
  (tool/version + origin + per-severity breakdown);
- One `section` per finding, worst-first, each with an `activityTitle` (index
  + `[severity/category] title`), a `facts` definition list (severity,
  category, confidence, contract, detector, location as `file:line`), and a
  ...
  "sections": [
    {
      ...
      "facts": [
        ...
        {"name": "Category", "value": "reentrancy"},
        {"name": "Confidence", "value": "high"},
        {"name": "Contract", "value": "MyContract"},
        {"name": "Detector", "value": "slither:reentrancy-eth"},
        {"name": "Location", "value": "MyContract.sol:42"}
      ],
      "markdown": true,
...
A bytecode-mode finding (no source mapping) emits its section without the
`Location` fact — the finding still appears in the channel card under its
severity / category / contract / detector facts, parallel to how the
`gha`/`azure-devops` formatters degrade for the same case. An empty (clean)
scan emits a single green card with no sections and a "no findings" body, so
the channel still gets a visible "omen ran, found nothing" trace rather than
silent empty output.

In a CI YAML, combine it with [`--fail-on`](#failing-ci-on-findings) so the
card is posted on every run and the pipeline blocks only when a high/critical
lead is present:

```yaml
- name: omen scan -> Teams
  env:
    TEAMS_WEBHOOK_URL: ${{ secrets.TEAMS_WEBHOOK_URL }}
  run: |
    omen --contract MyContract.sol --input-type sol --check all \
      --format teams-webhook \
    | tee /tmp/omen-card.json \
    | curl -H 'Content-Type: application/json' -d @- "$TEAMS_WEBHOOK_URL"
    # second invocation gates the pipeline on severity (the card already posted)
    omen --contract MyContract.sol --input-type sol --check all --fail-on high
```

Like the other scan formats, `teams-webhook` applies to single-`--contract`
mode; a `--batch` run always emits its JSONL stream. The payload is a single
self-contained JSON object — combine it with `-o/--output-file` to capture
the card to disk for an artifact in addition to (or instead of) posting it.

### Opsgenie / Atlassian Ops alert output

[Opsgenie](https://www.atlassian.com/software/opsgenie) (now Atlassian Ops)
is the on-call alerting platform used by security and DevOps teams to route,
escalate, and acknowledge alerts. `--format opsgenie` emits a single JSON
document matching the
[Opsgenie Create Alert API](https://docs.opsgenie.com/docs/alert-api#create-alert)
schema. A CI step pipes it straight to the API endpoint and the scan result
becomes a trackable, routable, acknowledgeable alert in the on-call queue —
with no Power Automate flow, no additional service, and no Marketplace
extension.

```bash
omen --contract MyContract.sol --input-type sol --check all --format opsgenie \
...
For EU instances use `https://api.eu.opsgenie.com/v2/alerts`.

The alert payload carries:

- **`message`** — a short headline (`"omen security scan: N findings"`) capped
  at 130 characters (the Opsgenie API limit), used for notification subjects
  and the alert-list preview.
- **`alias`** — a stable deduplication key (`omen/<origin>/<input_type>`) so
  re-running the same scan does not open a duplicate alert; Opsgenie closes the
  existing one and replaces it with the new result.
- **`description`** — a full markdown body with the scan context and one section
  per finding (severity, category, confidence, contract, detector, location, and
  description body).
- **`priority`** — `P1`..`P5` projected from the *worst* severity in the report
  (`critical`→P1, `high`→P2, `medium`→P3, `low`→P4, `informational`→P5), so
  the on-call rotation is paged at the appropriate urgency.
- **`tags`** — one `omen:<category>` tag per finding category, so Opsgenie
  routing rules and alert policies can dispatch category-specific on-call
  escalations (e.g. route all `access-control` findings to the security
  team's rotation, `reentrancy` findings to the DeFi team).
- **`details`** — a string→string metadata map with `tool`, `version`,
  `origin`, `input_type`, `checks`, and `finding_count` for
  integration-level filtering and webhook forwarding.

An empty report still emits a P5-priority "no findings" alert (tagged
`omen:clean`) so the on-call integration gets a visible "omen ran, found
nothing" trace rather than silent empty output.

Like the other scan formats, `opsgenie` applies to single-`--contract` mode;
a `--batch` run always emits its JSONL stream.

### VictorOps / Splunk On-Call alert output

[VictorOps](https://victorops.com/) (now [Splunk On-Call](https://www.splunk.com/en_us/products/on-call.html))
is an on-call management platform that routes alerts to the right team based on
escalation policies and schedules. `--format victorops` emits a single JSON
document matching the
[VictorOps REST Endpoint](https://help.victorops.com/knowledge-base/victorops-restendpoint-integration/)
schema, ready to POST directly to a routing-key URL:

```bash
omen --contract MyContract.sol --input-type sol --check all --format victorops \
  | curl -s -X POST \
      -H "Content-Type: application/json" \
      -d @- \
      "https://alert.victorops.com/integrations/generic/<INTEGRATION_ID>/alert/<ROUTING_KEY>"
```

The payload fields:

- **`message_type`** — the on-call priority: `CRITICAL` (worst severity critical
  or high — pages the rotation), `WARNING` (medium), `INFO` (low or informational),
  or `RECOVERY` (empty report — automatically resolves any open alert for the same
  `entity_id`, so a clean re-scan clears the on-call queue with no manual close).
- **`entity_id`** — `omen/<origin>/<input_type>`, a stable deduplication key.
  VictorOps correlates on `entity_id` to update an existing incident rather than
  opening a new one on every re-scan.
- **`entity_display_name`** — a short headline ("omen: N findings in MyContract.sol")
  surfaced in VictorOps' alert list and mobile push notifications.
- **`state_message`** — the full numbered finding summary as plain text (severity,
  category, location, and a one-line description synopsis per finding, capped at
  20 KB), the body an on-call responder reads in the alert detail view.
- **`monitoring_tool`** — `"omen"`, displayed in VictorOps' monitoring-tool field
  to distinguish omen alerts from other integrations in the same on-call timeline.

Like the other scan formats, `victorops` applies to single-`--contract` mode;
a `--batch` run always emits its JSONL stream.
